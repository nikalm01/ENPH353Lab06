...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        #Start of code to detect line

        gray_frame = cv2.cvtColor(cv_image, cv2.COLOR_RGB2GRAY)
        threshold = 130
        ret, binary_frame = cv2.threshold(gray_frame, threshold, 255, cv2.THRESH_BINARY)

        height = binary_frame.shape[0]
        width = binary_frame.shape[1]
        section_width = int(width/10)

        line_of_analysis = 3*height/4 + 10
        ...
        for i in range(width):
            ...
            if binary_frame[line_of_analysis, i] < threshold:
        ...
        if (line_count != 0):
            x_centroid = int(xpos_sum/line_count)
            line_position = int(x_centroid/section_width)
            state[line_position] = 1
            self.timeout = 0
